...
def hlp(message):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(types.KeyboardButton('/start'), types.KeyboardButton('/register'), types.KeyboardButton('/stats'),
               types.KeyboardButton('/encrypt'), types.KeyboardButton('/decrypt'))
    bot.send_message(message.chat.id,
...
def encrypt_method(message):
    if message.text == 'Цезарь':
        inpt = input_bot(message, "Введите данные для шифрования [строка] [сдвиг]")
        inpt, key = inpt.split(' ')
        enc = MessageEncrypter(inpt)
        bot.send_message(message.chat.id, enc.caesar_encrypt(int(key)))
    else:
        inpt = input_bot(message, "Введите данные для шифрования [строка] [ключ-пароль]")
        inpt, key = inpt.split(' ')
        enc = MessageEncrypter(inpt)
...
def decrypt_method(message):
    if message.text == 'Цезaрь':
        inpt = input_bot(message, "Введите данные для дешифрования [шифр] [сдвиг]")
        inpt, key = inpt.split(' ')
        enc = MessageEncrypter(inpt)
        bot.send_message(message.chat.id, enc.caesar_decrypt(int(key)))
    else:
        inpt = input_bot(message, "Введите данные для дешифрования [шифр] [ключ-пароль]")
        inpt, key = inpt.split(' ')
        enc = MessageEncrypter(inpt)
...
def main_menu(message):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(types.KeyboardButton('/help'), types.KeyboardButton('/register'), types.KeyboardButton('/stats'),
               types.KeyboardButton('/encrypt'), types.KeyboardButton('/decrypt'))
